...
import Russoul.lib.common.math.geometry.simple.{PlaneOverES, RectangleOverES, SphereOverES}
...
class Frustum(val pos: Vec3[Real], val angleOfView: Real, val aspect: Real, val lookingDir: Vec3[Real], val zNear: Real, val zFar: Real, val up: Vec3[Real])
{
  ...
  def genSphericalBound(near: RectangleOverES[Real3,Real], far: RectangleOverES[Real3, Real]): SphereOverES[Real3, Real] =
  {
  ...
  /**
    ...
    */
  def genNearPlane(): RectangleOverES[Real3, Real] =
  {
    ...
    val nearPlane:RectangleOverES[Real3, Real] = new RectangleOverES(center, ext1v, ext2v)
  ...
  /**
    ...
    * @return facing the camera, out of the frustum
    */
  def genFarPlane(): RectangleOverES[Real3, Real] =
  {
    ...
    val farPlane = new RectangleOverES[Real3, Real](center, ext1v, ext2v)
  ...
  /**
    ...
    */
  def genExtraPlanes(): Arr[PlaneOverES[Real3, Real]] =
  {
    val n = genNearPlane()
    val f = genFarPlane()
    ...
    Arr[PlaneOverES[Real3, Real]](PlaneOverES(nlb, normalLeft), PlaneOverES(nrb, normalRight), PlaneOverES(frt, normalTop), PlaneOverES(flb, normalBottom))
  }
}
