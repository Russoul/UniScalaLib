...
import Russoul.lib.common.math.geometry.simple.Rectangle
...
/**
  ...
  */
class RenderRectangleTextured
{
  private final val VERTEX_SIZE: Int = 5
  ...
  final val texCoords = Array[Float](0,0, 1,0, 1,1, 0,1)

  def addRectangle(rec:Rectangle): Unit =
  {
    ...
    for(i <- v.indices)
    {
      ...
      vertexPool += texCoords(i*2 + 0)
      vertexPool += texCoords(i*2 + 1)
    }
