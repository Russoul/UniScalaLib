...
import Russoul.lib.common.math.immutable.geometry.simple.Triangle
...
class RenderTriangle
{
  ...
  def addTriangle(tr:Triangle, color:vec3): Unit =
  {

    vertexPool ++= tr.p1.toArray3f()
    vertexPool ++= color.toArray3f()

    vertexPool ++= tr.p2.toArray3f()
    vertexPool ++= color.toArray3f()

    vertexPool ++= tr.p3.toArray3f()
    vertexPool ++= color.toArray3f()

    val is = Array[Int](0+vcount,1+vcount,2+vcount)
    indexPool ++= is
