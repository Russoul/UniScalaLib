...
@immutable case class PlaneOver[@tbsp F]private(val point:Vec3[F],val normal:Vec3[F]) extends GeometricShape[F,_3]{
  ...
  override def translate(v: Vec3[F])(implicit ev3: Field[F]): PlaneOver[F] = new PlaneOver[F](point, normal)


  override def scaleAroundBasis(factor: F)(implicit ev3: Field[F]): PlaneOver[F] = {
